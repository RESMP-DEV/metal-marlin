...
#pragma warning(disable                                                        \
                : 4293) // shift count negative or too big  <-- erroneously
                        // raised in a constexpr-disabled block
#pragma warning(disable : 4310) // cast truncates constant value <-- erroneously
                                // raised in a constexpr-disabled block
#endif
...
#define check(cond, ...)                                                       \
  if (NB_UNLIKELY(!(cond)))                                                    \
  nanobind::detail::fail(__VA_ARGS__)
#endif
...
struct nb_func {
  PyObject_VAR_HEAD PyObject *(*vectorcall)(PyObject *, PyObject *const *,
                                            size_t, PyObject *);
...
struct nb_ndarray {
  PyObject_HEAD ndarray_handle *th;
};

/// Python object representing an `nb_method` bound to an instance (analogous to
/// non-public PyMethod_Type)
struct nb_bound_method {
  PyObject_HEAD PyObject *(*vectorcall)(PyObject *, PyObject *const *, size_t,
                                        PyObject *);
  nb_func *func;
  PyObject *self;
};
...
// Linked list of type aliases when there are multiple shared libraries with
// duplicate RTTI data
...
/// Convenience functions to deal with the pointer encoding in
/// 'internals.inst_c2p'
...
NB_INLINE nb_inst_seq *nb_get_seq(void *p) {
  return (nb_inst_seq *)(((uintptr_t)p) ^ 1);
}
 ...
 */
#if defined(NB_FREE_THREADED)
template <typename T> struct nb_maybe_atomic {
...
#if !defined(NB_FREE_THREADED)
  /// C++ -> Python type map -- fast version based on std::type_info pointer
  /// equality
...
#if !defined(NB_FREE_THREADED)
  /// nb_func/meth instance map for leak reporting (used as set, the value is
  /// unused) In free-threaded mode, functions are immortal and don't require
  /// this data structure.
  nb_ptr_map funcs;
#endif
...
struct lock_internals {
  lock_internals(nb_internals *) {}
};
struct unlock_internals {
  unlock_internals(nb_internals *) {}
};
struct lock_obj {
  lock_obj(PyObject *) {}
};
