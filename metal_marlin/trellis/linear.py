...
class TrellisLinear(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "packed_indices",
            torch.zeros(tiles_k, tiles_n, packed_bytes,
                        dtype=torch.uint8, device=device),
        )
        self.register_buffer(
            "scales",
            torch.ones((in_features + 127) // 128, out_features,
                       dtype=torch.float32, device=device),
        )
        self.register_buffer("su", torch.ones(
            in_features, dtype=torch.float32, device=device))
        self.register_buffer("sv", torch.ones(
        ...
        if bias:
            self.register_buffer("bias", torch.zeros(
                out_features, dtype=torch.float16, device=device))
        else:
            self.bias = None

        # Mixed BPW: bits_per_group buffer
        self.register_buffer("bits_per_group", torch.full(
            (tiles_k,), fill_value=bits, dtype=torch.int8, device=device))
    ...
    def _pack_same_bitwidth_groups_contiguously(
        ...
        for original_idx in sorted_group_indices:
            ...
            scale_tensors.append(
                scales_by_bits[bits][group_num_in_batch, :].unsqueeze(0))
    ...
    def from_mixed_bpw_tensors(
        ...
        module = cls(in_features, out_features, bits=max_bits,
                     bias=bias is not None, device=device)
        ...
        module.packed_indices = nn.Parameter(
            packed_indices.to(device), requires_grad=False)
        ...
        module.bits_per_group = nn.Parameter(bits_per_group.to(
            device=device, dtype=torch.int8), requires_grad=False)
    ...
    def forward_mixed_bpw(self, x: torch.Tensor) -> torch.Tensor:
        ...
        output = torch.zeros(M, self.out_features,
        ...
        for bits in layout.unique_bits:
            ...
            if bits == 8:
                ...
                for contig_group_idx in groups_for_bit:
                    ...
                    end_feature = min(
                        start_feature + group_size, self.in_features)
                ...
                weight_slices = [self._full_weight_dequant[:, start:end]
                ...
                for contig_group_idx in groups_for_bit:
                    ...
                    packed_slice = self.packed_indices.view(
                        -1)[start_offset:end_offset]
                    packed_list.append(packed_slice)
                    scales_list.append(
                        self.scales[contig_group_idx:contig_group_idx + 1])
                ...
                total_features = sum(end - start for start,
                                     end in feature_ranges)
                ...
                packed_reshaped = packed_concat.view(
                    num_groups_bit, 1, packed_bytes)
    ...
    def from_trellis_weight(
        ...
        # Create module with basic init (wrong buffer sizes, will replace)
        module = cls.__new__(cls)
        super(TrellisLinear, module).__init__()

        # Set attributes
        module.in_features = weight.original_shape[1]
        module.out_features = weight.original_shape[0]
        module.bits = weight.bits

        # Mixed BPW support
        module._is_mixed_bpw = False
        module._mixed_bpw_layout = None
        module._mixed_bpw_unique_bits = []
        module._mixed_bpw_bit_to_groups = {}
        module._lib = None
        ...
        module.register_buffer(
            "packed_indices", weight.packed_indices.to(device))
        ...
        if weight.bits > 0:
            ...
            grid = torch.from_numpy(codebook.get_grid()).float()
            module.register_buffer("grid", grid.to(device))
        ...
        # Mixed BPW: bits_per_group buffer (create with correct size)
        tiles_k = weight.packed_indices.shape[0]
        module.register_buffer(
            "bits_per_group",
            torch.full((tiles_k,), fill_value=weight.bits,
    ...
    def dequantize(self) -> torch.Tensor:
        ...
        if HAS_METAL and HAS_MPS and self.packed_indices.is_mps:
            try:
                ...
                import warnings
                warnings.warn(
                    f"Metal dequantize failed, falling back to CPU: {e}", RuntimeWarning)
    ...
    def _dequantize_cpu_packed(self, K: int, N: int, group_size: int) -> torch.Tensor:
        ...
        for tile_k in range(tiles_k):
            for tile_n in range(tiles_n):
                indices = self._unpack_tile_indices(
                    packed_indices[tile_k, tile_n], self.bits, len(grid))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if HAS_METAL and HAS_MPS and x.is_mps:
            ...
            try:
                ...
                import warnings
                warnings.warn(
                    f"Metal TrellisLinear dispatch failed: {e}", RuntimeWarning)
