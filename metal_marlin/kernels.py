...
def _load_attention_exports_module() -> ModuleType:
    logger.info("_load_attention_exports_module called")
    module_name = "metal_marlin._kernels_attention"
    cached = sys.modules.get(module_name)
    if cached is not None:
        return cached

    module_path = Path(__file__).resolve().parent / "kernels" / "attention.py"
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load attention kernel module from {module_path}")
...
def _load_moe_exports_module() -> ModuleType:
    logger.info("_load_moe_exports_module called")
    module_name = "metal_marlin._kernels_moe"
    cached = sys.modules.get(module_name)
    if cached is not None:
        return cached

    module_path = Path(__file__).resolve().parent / "kernels" / "moe.py"
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load MoE kernel module from {module_path}")
...
if HAS_METAL and HAS_MPS:
    ...
    _attention_exports = _load_attention_exports_module().build_attention_exports(
        np=np,
        torch=torch,
        require_mps=require_mps,
        get_default_library=get_default_library,
        _ensure_kernel_compiled=_ensure_kernel_compiled,
        _private_buffer_from_tensor=_private_buffer_from_tensor,
        _params_buffer=_params_buffer,
        mps_tensor_to_metal_buffer=mps_tensor_to_metal_buffer,
        dispatch_kernel=dispatch_kernel,
        FP4_PER_UINT=FP4_PER_UINT,
    )
    flash_attention_kv_fp4 = _attention_exports["flash_attention_kv_fp4"]
    paged_attention_fp4 = _attention_exports["paged_attention_fp4"]
    paged_attention_v1 = _attention_exports["paged_attention_v1"]
    quantized_kv_attention_decode = _attention_exports["quantized_kv_attention_decode"]
    ...
    globals().update(
        _load_moe_exports_module().build_moe_exports(
            np=np,
            torch=torch,
            require_mps=require_mps,
            ...
            mps_tensor_to_metal_buffer=mps_tensor_to_metal_buffer,
            dispatch_kernel=dispatch_kernel,
            get_shader_source=get_shader_source,
            marlin_gemm_fp4=marlin_gemm_fp4,
