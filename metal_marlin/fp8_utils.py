"""FP8 quantization utilities for KV cache.

Provides PyTorch-based FP8 E4M3 and E5M2 quantization/dequantization for memory-efficient
...
FP8 E4M3 Format:
    ...
    - Higher precision, preferred for weights

FP8 E5M2 Format:
    - 1 sign bit, 5 exponent bits (bias=15), 2 mantissa bits
    - Range: ~2^-16 to 57344 (with infinity)
    - 256 distinct values including Inf/NaN
    - Wider dynamic range, preferred for activations/gradients in training

Usage:
    from metal_marlin.fp8_utils import (
        quantize_to_fp8_e4m3, dequantize_fp8_e4m3,
        quantize_to_fp8_e5m2, dequantize_fp8_e5m2,
    )

    # Quantize K, V for storage (E4M3 - higher precision)
    ...
    # Or use E5M2 for wider dynamic range (training compatibility)
    k_fp8, k_scales = quantize_to_fp8_e5m2(k_fp16, scale_method="channel")
    v_fp8, v_scales = quantize_to_fp8_e5m2(v_fp16, scale_method="channel")
...
# Max representable finite value: 2^15 * (1 + 3/4) = 32768 * 1.75 = 57344
FP8_E5M2_MAX: float = 57344.0

# Minimum positive subnormal: 2^-16 = 1.52587890625e-05
FP8_E5M2_MIN_POSITIVE: float = 2**-16
...
# =============================================================================
# FP8 E5M2 Format Support (Training-Compatible)
# =============================================================================


def _compute_fp8_e5m2_codebook() -> np.ndarray:
...
# Precomputed E5M2 codebook for dequantization lookup
_FP8_E5M2_CODEBOOK: np.ndarray = _compute_fp8_e5m2_codebook()

# PyTorch version of E5M2 codebook (lazily initialized)
_FP8_E5M2_CODEBOOK_TORCH: torch_typing.Tensor | None = None


def _get_torch_codebook_e5m2(device: str = "cpu") -> torch_typing.Tensor:
    """Get the FP8 E5M2 codebook as a PyTorch tensor.
    ...
    if _FP8_E5M2_CODEBOOK_TORCH is None:
        _FP8_E5M2_CODEBOOK_TORCH = torch.from_numpy(_FP8_E5M2_CODEBOOK)

    return _FP8_E5M2_CODEBOOK_TORCH.to(device)
...
def quantize_to_fp8_e5m2(
    ...
    """Quantize FP16/BF16 tensor to FP8 E5M2.

    Computes per-element or per-group scales and quantizes to uint8 storage
    representing FP8 E5M2 codes. The quantization is simulated using the
    FP8 E5M2 codebook with nearest-neighbor matching.

    FP8 E5M2 provides wider dynamic range than E4M3 (57344 vs 448 max value),
    making it more suitable for activations and gradients in training scenarios.
    ...
    Returns:
        fp8_tensor: uint8 tensor with FP8 E5M2 codes, same shape as input.
        scales: FP16 scales for dequantization. Shape depends on scale_method:
            - "tensor": [1] or scalar broadcastable
            - "channel": [..., 1] (scale per position in last dim)
            - "group": [..., num_groups] where num_groups = last_dim / group_size
    ...
    Example:
        >>> k = torch.randn(1, 8, 1024, 128, dtype=torch.float16, device="mps")
        >>> k_fp8, k_scales = quantize_to_fp8_e5m2(k, scale_method="channel")
        >>> k_fp8.shape  # Same as input
        torch.Size([1, 8, 1024, 128])
        >>> k_fp8.dtype
        torch.uint8
    ...
    if scale_method == "tensor":
        ...
        # Quantize with expanded scales
        scaled = x / scale_expanded
        scaled = scaled.clamp(-FP8_E5M2_MAX, FP8_E5M2_MAX)

        # Get the codebook for nearest-neighbor quantization
        codebook = _get_torch_codebook_e5m2(device)

        # Find nearest FP8 code for each value
        fp8_tensor = _quantize_tensor_to_fp8_e5m2(scaled, codebook)
...
def _quantize_tensor_to_fp8_e5m2(
    scaled: torch_typing.Tensor,
    codebook: torch_typing.Tensor,
) -> torch_typing.Tensor:
    """Quantize scaled tensor to FP8 E5M2 codes using nearest-neighbor.

    Uses a fast linear approximation for normal values and handles
    edge cases (zero, subnormal) correctly.

    Args:
        scaled: Tensor with values in [-57344, 57344] range.
        codebook: FP8 E5M2 codebook tensor of shape [256].
    ...
    # Get valid (finite) codes for nearest-neighbor matching
    # Filter out Inf/NaN codes (E=31)
    valid_mask = torch.isfinite(codebook)
    valid_indices = torch.where(valid_mask)[0]
    valid_values = codebook[valid_mask]

    # For large tensors, process in chunks to avoid memory explosion
    # from the [N, 256] distance matrix
    flat = scaled.reshape(-1)
    n = flat.shape[0]
    chunk_size = 1 << 18  # ~256K elements per chunk
    ...
    for start in range(0, n, chunk_size):
        ...
        # Compute distances to all valid FP8 values
        # chunk: [chunk_size], valid_values: [~256]
        # distances: [chunk_size, num_valid]
        distances = (chunk.unsqueeze(1) - valid_values.unsqueeze(0)).abs()
...
def dequantize_fp8_e5m2(
    ...
    """Dequantize FP8 E5M2 to FP16/BF16/FP32.

    Performs codebook lookup to convert uint8 FP8 E5M2 codes back to float values,
    then applies the quantization scales.

    Args:
        fp8_tensor: uint8 tensor with FP8 E5M2 codes.
        scales: Quantization scales from quantize_to_fp8_e5m2. Shape must
            broadcast against fp8_tensor.
        output_dtype: Output dtype. Defaults to torch.float16 if None.
    ...
    Example:
        >>> k_fp8, k_scales = quantize_to_fp8_e5m2(k, scale_method="channel")
        >>> k_restored = dequantize_fp8_e5m2(k_fp8, k_scales)
        >>> k_restored.dtype
        torch.float16
    """
    require_torch()
...
def quantize_kv_cache_fp8_e5m2(
    ...
    """Quantize both K and V tensors for KV cache storage using FP8 E5M2.

    Convenience function that quantizes both key and value tensors using
    the same scale method. E5M2 provides wider dynamic range than E4M3,
    making it suitable for training scenarios.

    Args:
        k: Key tensor [batch, num_kv_heads, seq_len, head_dim]
        v: Value tensor [batch, num_kv_heads, seq_len, head_dim]
        scale_method: Scale computation method (see quantize_to_fp8_e5m2)
...
def dequantize_kv_cache_fp8_e5m2(
    ...
    """Dequantize K and V tensors from FP8 E5M2 storage.
...
def compute_quantization_error_e5m2(
    original: torch_typing.Tensor,
    scale_method: Literal["tensor", "channel", "group"] = "channel",
    group_size: int = 128,
) -> dict[str, float]:
    """Compute quantization error metrics for FP8 E5M2.

    Useful for evaluating the impact of FP8 E5M2 quantization on model quality.
    ...
    fp8, scales = quantize_to_fp8_e5m2(original, scale_method, group_size)
    restored = dequantize_fp8_e5m2(fp8, scales, original.dtype)
