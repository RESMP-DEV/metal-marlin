...
class HeaderPrefix:
    ...
    def __init__(self, file):
        logger.debug("initializing %s with file=%s", type(self).__name__, file)
        self.__file = file
    ...
    def __get_commit_hash(self):
        logger.debug("__get_commit_hash called")
        git_commit_hash = None

        try:
            git_dir = os.path.dirname(os.path.realpath(__file__))
            proc = subprocess.Popen(
                ['git', 'rev-parse', 'HEAD'], cwd=git_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def __get_commit_string(self):
        logger.debug("__get_commit_string called")
        meta_data = None
    ...
    def __get_date_string(self):
        logger.debug("__get_date_string called")
        today = datetime.date.today()

        return today.strftime(self.__template_date)

    def __meta_data_string(self):
        logger.debug("__meta_data_string called")
...
class SingleHeader:
    ...
    def append(self, header_path):
        logger.debug("append called with header_path=%s", header_path)
    ...
    def __read_header(self, path):
        logger.debug("__read_header called with path=%s", path)
    ...
    def __strip_pragma_once(self, header):
        logger.debug("__strip_pragma_once called with header=%s", header)
        return re.sub('\\s*#pragma once\\s*\\/\\/-*\\n', '', header)

    def __strip_comments(self, header):
        logger.debug("__strip_comments called with header=%s", header)
        return re.sub('^//.*\\n', '', header, flags=re.MULTILINE)

    def __strip_empty_lines(self, header):
        logger.debug("__strip_empty_lines called with header=%s", header)
        return re.sub('\\n\\n+', '\\n\\n', header, flags=re.MULTILINE)

    def __substitute_include_directive(self, match):
        logger.debug("__substitute_include_directive called with match=%s", match)
    ...
    def __process_include_directives(self, header):
        logger.debug("__process_include_directives called with header=%s", header)
    ...
    def __process_foundation_directives(self, header):
        logger.debug("__process_foundation_directives called with header=%s", header)
    ...
    def __process_header(self, header_path):
        logger.debug("__process_header called with header_path=%s", header_path)
        out_header = ''

        header_path = os.path.realpath(header_path)

        if header_path not in self.__included_headers:
...
def create_argument_parser():
    logger.debug("create_argument_parser called")
    parser = argparse.ArgumentParser()
    base_path = os.path.dirname(os.path.realpath(__file__))
    output_path = os.path.join(base_path, 'Metal.hpp')

    parser.add_argument('-o', '--output',  dest='output_path', metavar='PATH',
                        default=output_path, help='Output path for the single header file.')
    parser.add_argument('-v', '--verbose', action='store_true',
                        help='Show verbose output.')
    parser.add_argument(dest='header_paths', metavar='HEADER_FILE',
                        nargs='+', help='Input header file.')
...
def parse_arguments():
    logger.debug("parse_arguments called")
...
def make_header(args):
    logger.debug("make_header called with args=%s", args)
...
def make_dir(path):
    logger.debug("make_dir called with path=%s", path)
    try:
...
def write_header(args, content):
    logger.info("write_header called with args=%s, content=%s", args, content)
